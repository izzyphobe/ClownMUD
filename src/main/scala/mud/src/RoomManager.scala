...
class RoomManager extends Actor {
  ...
  def receive = {
    ...
    case ShortestPath(sender, start, end, dirs) =>
      val source = XML.loadFile("src/main/scala/mud/src/map.xml")
      val st=(source\\"@room").find(i=>(i\@"room")==start)
      val exits = for (room <- (source \\ "room")) yield {
        (room \\ "@room").text
      }
    //      if (dirs.length < 10) {
    //        println(dirs)
    //        rooms(start) ! Room.HasPath(sender, end, dirs)
    //      }
    case FoundPath(sender, room, path) =>
      sender ! Player.GetChat("Found path to " + room.toString() + ": " + path.mkString(", ").toString())

    case StartShortestPath(sender: ActorRef, start: ActorRef, target: String) =>
      if (!(rooms.keys.toSeq.contains(target))) {
        sender ! Player.GetChat("That room doesn't exist! Type 'map' for a full list of rooms.")
      } else {
        for (room <- rooms) {
          if (room._2 == start) {
            self ! ShortestPath(sender, room._1, target, Array(""))
          }
        }
      }
...
object RoomManager {
  ...
  case object Begin
  case class FoundPath(sender: ActorRef, room: String, path: Array[String])
  case class ShortestPath(sender: ActorRef, room: String, target: String, path: Array[String])
  case class StartShortestPath(sender: ActorRef, start: ActorRef, target: String)
